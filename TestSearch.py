...
class SAR_A_Star_Square:
    ...
    def f3_probability_gain(self, next_pos):
        ...
        return 1.0 - self.p_DT_map[next_pos[0], next_pos[1]]*0.825
    ...
    def save_visit_order(self, filename="visit_order.csv"):
        """
        将访问顺序保存到本地 CSV 文件中，格式为：
        Order, X, Y
        """
        # 将 visit_order 转换为按顺序排序的列表
        sorted_visits = sorted(self.visit_order.items(), key=lambda item: item[1])
        ...
        with open(filename, mode='w', newline='') as csv_file:
            ...
            # 写入每一行数据
            i=0
            for (pos, order) in sorted_visits:
                row, col = pos
                writer.writerow([i, col, row])  # X = column, Y = row
                i=i+1

        print(f"访问顺序已保存到 {filename}")
    ...
    def run_demo(self):
        ...
        # 保存访问顺序到本地 CSV 文件，文件名包含邻接模式信息
        filename = f"visit_order_{self.neighbor_mode}.csv"
        self.save_visit_order(filename)
